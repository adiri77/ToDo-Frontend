...
function Task() {
    const onDragEnd = (result, columns, setColumns) => {
        ...
        if (source.droppableId !== destination.droppableId) {
            ...
            // Set the completed timestamp when the task is moved to "Done"
            if (destColumn.name === "Done") {
                removed.completedAt = new Date().toLocaleDateString('en-GB'); // Format: DD/MM/YY
                removed.stage = "Done";
            }
    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            axios.get(`https://todo-backend-i533.onrender.com/project/${projectId}`)
                .then((res) => {
    ...
    const updateTodo = (data) => {
        axios.put(`https://todo-backend-i533.onrender.com/project/${projectId}/todo`, data)
            .then((res) => {
            }).catch((error) => {
                toast.error('Something went wrong');
            });
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`https://todo-backend-i533.onrender.com/project/${projectId}/task/${taskId}`)
            .then((res) => {
    ...
    const moveTaskToNextStage = (task) => {
        const stageOrder = ["Requested", "In Progress", "Done"];
        const currentStageIndex = stageOrder.indexOf(task.stage);
        if (currentStageIndex === stageOrder.length - 1) return; // If the task is already in "Done", do nothing.

        const nextStage = stageOrder[currentStageIndex + 1];
        const updatedTask = { ...task, stage: nextStage };

        // Set the completed timestamp when the task reaches "Done"
        if (nextStage === "Done") {
            updatedTask.completedAt = new Date().toLocaleDateString('en-GB'); // Format: DD/MM/YY
        }

        const updatedColumns = { ...columns };
        const sourceColumnKey = Object.keys(columns).find(key => columns[key].name === task.stage);
        const destinationColumnKey = Object.keys(columns).find(key => columns[key].name === nextStage);

        if (sourceColumnKey && destinationColumnKey) {
            updatedColumns[sourceColumnKey].items = updatedColumns[sourceColumnKey].items.filter(item => item._id !== task._id);
            updatedColumns[destinationColumnKey].items.push(updatedTask);
                                                    ...
                                                    {column.items.map((item, index) => {
                                                        return (
                                                            <Draggable
                                                                ...
                                                                index={index}
                                                                isDragDisabled={item.stage === "Done"} // Disable dragging for completed tasks
                                                            >
                                                                {(provided, snapshot) => {
                                                                    return (
                                                                        <div
                                                                            ...
                                                                            {...(item.stage !== "Done" ? provided.dragHandleProps : {})}
                                                                        ...
                                                                        >
                                                                            <div className="pb-2">
                                                                                ...
                                                                                {item.stage === "Done" ? (
                                                                                    <span className="block mt-2 text-xs text-gray-600">Completed on: {item.completedAt}</span>
                                                                                ) : (
                                                                                    <button
                                                                                        className="mt-2 text-xs text-white bg-indigo-600 hover:bg-indigo-700 rounded-md px-2 py-1 ml-7"
                                                                                        onClick={(e) => {
                                                                                            e.stopPropagation();
                                                                                            moveTaskToNextStage(item);
                                                                                        }}
                                                                                    >
                                                                                        Move to Next Stage
                                                                                    </button>
