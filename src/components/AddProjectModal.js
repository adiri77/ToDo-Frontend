...
const AddProjectModal = ({ isModalOpen, closeModal, edit = false, id = null }) => {
    ...
    useEffect(() => {
        if (edit && isModalOpen) {
            axios.get(`https://todo-backend-i533.onrender.com/project/${id}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post('https://todo-backend-i533.onrender.com/project/', { title, description: desc })
                .then((res) => {
                    closeModal()
            ...
            axios.put(`https://todo-backend-i533.onrender.com/project/${id}`, { title, description: desc })
                .then((res) => {
                    closeModal()
                    const customEvent = new CustomEvent('projectUpdate', { detail: { ...res.data } });
                    document.dispatchEvent(customEvent);
